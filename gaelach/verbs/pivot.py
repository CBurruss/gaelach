...
def pivot_wider(names_from, values_from=None, id_cols=None, 
    ...
    def _pivot_wider(df):
        ...
        agg_func = agg_map.get(values_fn, values_fn)
        ...
        if isinstance(result.columns, pd.MultiIndex):
            result.columns = [names_sep.join(map(str, col)).strip(names_sep) 
        ...
        if sort_columns:
            # Sort pivoted columns but keep id_columns in original order
            if id_columns:
                pivoted_cols = [col for col in result.columns if col not in id_columns]
                result = result[id_columns + sorted(pivoted_cols)]
            else:
                result = result[sorted(result.columns)]
        
        # Reset index to make id_columns regular columns
...
def _resolve_pivot_cols(cols, df):
    ...
    # Handle ColumnRange (_.col1 | _.col2 syntax)
        ...
        cols_to_pivot = list(df.columns[start_idx:end_idx + 1])
